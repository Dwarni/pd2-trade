...
export function RunePricePopover({
  ...
  const highRunes = calculatedRuneValues.filter(
    (rune) => !['Gul Rune', 'Ist Rune', 'Mal Rune', 'Um Rune', 'Pul Rune', 'Lem Rune'].includes(rune.name),
  );
              ...
              <Tooltip delayDuration={0}>
                ...
                <TooltipContent>
                  <p className="max-w-xs">
                    Rune prices are calculated from market listings using the median price over the last 7 days. Prices
                    are updated daily.
                  </p>
                </TooltipContent>
              </Tooltip>
            ...
            <div className="space-y-1">
              {displayedRunes.map((rune) => (
                <div key={rune.name}
                  ...
                  <div className="flex items-center gap-2">
                    <span className={cn('font-medium', rune.isFixed ? 'text-gray-400' : 'text-green-500')}>
                      {rune.price} HR
              ...
              {selectedRuneBreakdown && selectedRuneCombinations.length > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-600">
                  <h5 className="font-semibold text-sm mb-2">{selectedRuneBreakdown} Breakdown:</h5>
                  <div className="space-y-2">
                    {selectedRuneCombinations.map((combo, index) => {
                      const targetValue =
                        calculatedRuneValues.find((r) => r.name === selectedRuneBreakdown)?.price || 0;
                      return (
                        <div key={index}
              ...
              <div className="text-xs text-gray-400 mt-2 pt-2 border-t border-gray-600">
                * Fixed pricing used with Vex and below, as it generally does not change or when &lt;10 listings
                available
              </div>
            </div>
          )}
