...
const ItemStatsDisplay: React.FC<ItemStatsDisplayProps> = ({ stashItem, isExpanded, onToggleExpanded }) => {
  ...
  const nonCorruptedModifiers = filteredModifiers.filter(
    (mod) => !(stashItem.corruptions?.includes(mod.name) ?? false),
  );
  ...
  return (
    <div className="text-xs space-y-1">
      <div className="flex flex-row gap-1">
        {stashItem.is_ethereal && <Badge className="text-xs rounded-lg">Ethereal</Badge>}
        {stashItem.corrupted && (
        ...
        {stashItem.socket_count > 0 && (
          <Badge variant="secondary"
            className="text-xs rounded-lg">
            <Badge className="rounded-4xl text-xs h-4 w-4">{stashItem.socket_count}</Badge>
        ...
        <div className="mt-2">
          {isExpanded ? (
            ...
            sortedModifiers.map((mod, idx) => (
              <div
                key={idx}
                className={cn('text-gray-200 truncate', {
                  'text-red-500': stashItem?.corruptions && stashItem.corruptions.includes(mod.name),
                })}
              >
                {mod.label}
